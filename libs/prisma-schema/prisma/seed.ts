...
import { Prisma, PrismaClient } from "../../prisma-client/src";
...
async function getProductSizes(params: {
  ...
  const offers = (await response.json()) as TResponse<TOffer>;
  return offers.data
    .map((offer) => ({
      keyCrmId: offer.id,
      name:
...
async function seedCategories() {
  ...
  const categories = (await response.json()) as TResponse<TCategory>;
...
async function seedProducts() {
  ...
  do {
    ...
    const result = (await response.json()) as TResponse<TProduct>;
